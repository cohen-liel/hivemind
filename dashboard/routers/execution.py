                ...
                if msg_type == "pong":
                    ...
                    try:
                        ...
                        logger.info(
                            ...
                            ws_id, len(_history), project_id, _conv_id,
                        )
                    except Exception as _hist_err:
                        logger.error(
                            "WebSocket [%s]: failed to load history for project %s: %s",
                            ws_id, project_id, _hist_err,
                        )
                        await ws.send_json(
                            {
                                "type": "error",
                    ...
                    try:
                        ...
                        if _task_queue:
                            ...
                            if _task_rec:
                                await ws.send_json(
                                    {"type": "task_status", **_task_rec.to_dict()}
                                )
                                continue
                        await ws.send_json(
                            {
