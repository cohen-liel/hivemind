...
(NOT 'CLAUDE_MODEL' or 'MAX_TURNS' — those don't exist):
  ...
  Paths: PROJECTS_BASE_DIR, STORE_DIR, SESSION_DB_PATH, _PROJECT_ROOT
  ...
  Strings: ORCHESTRATOR_SYSTEM_PROMPT, SOLO_AGENT_PROMPT, SESSION_DB_PATH
...
class TestPathConfigs:
    ...
    def test_session_db_path_is_string(self):
        assert isinstance(config.SESSION_DB_PATH, str)
        assert config.SESSION_DB_PATH.endswith("sessions.db")
...
class TestSystemPrompts:
    ...
    def test_solo_prompt_does_not_mention_delegate(self):
        assert "delegate" not in config.SOLO_AGENT_PROMPT.lower() or \
               "do NOT delegate" in config.SOLO_AGENT_PROMPT
