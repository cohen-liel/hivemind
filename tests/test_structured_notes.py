...
class TestGetRelevantNotes:
    def _populate(self, notes):
        notes.add_note(NoteCategory.DECISION, "Use PostgreSQL", "Chosen for reliability", "db", "t1", ["database"])
        notes.add_note(NoteCategory.API, "POST /login", "Auth endpoint", "backend", "t2", ["auth"])
        notes.add_note(NoteCategory.SCHEMA, "Users table", "id, email, hash", "db", "t3", ["database", "users"])
        notes.add_note(NoteCategory.GOTCHA, "Rate limit on API", "Max 100 req/min", "backend", "t4", ["api"])
...
class TestBuildNotesContext:
    ...
    def test_context_filtered_by_role(self, notes):
        ...
        notes.add_note(NoteCategory.SCHEMA, "Users table", "Schema details", "db", "t2", ["database"])
