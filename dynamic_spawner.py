...
class DynamicSpawner:
    ...
    def get_respawn_model(
        ...
        logger.info(
            ...
            task.id, current_model or "default", next_model, reason,
        )
        return next_model
    ...
    def _classify_failure(output: TaskOutput) -> FailureCategory:
        """Infer failure category from output issues/summary.

        Uses the same heuristics as contracts.classify_failure but works
        on TaskOutput directly.
        """
        text = " ".join(output.issues + [output.summary]).lower()

        if any(kw in text for kw in ("timeout", "timed out", "max turns", "budget")):
            return FailureCategory.TIMEOUT
        if any(kw in text for kw in ("syntax", "compile", "build", "import error", "type error")):
            return FailureCategory.BUILD_ERROR
        if any(kw in text for kw in ("test fail", "assertion", "expect")):
            return FailureCategory.TEST_FAILURE
        if any(kw in text for kw in ("api mismatch", "contract", "endpoint", "schema")):
            return FailureCategory.API_MISMATCH
        if any(kw in text for kw in ("permission", "access denied", "forbidden")):
            return FailureCategory.PERMISSION
        if any(kw in text for kw in ("unclear", "ambiguous", "missing requirement")):
            return FailureCategory.UNCLEAR_GOAL
        if any(kw in text for kw in ("dependency", "missing", "not found", "upstream")):
            return FailureCategory.DEPENDENCY_MISSING

        # Default to BUILD_ERROR (eligible for model switch)
        return FailureCategory.BUILD_ERROR
