...
class MessageSchema(BaseModel):
    ...
    metadata: dict | None = Field(default=None, description="Optional metadata (model, tokens, …)")
...
async def get_project_history(
    ...
    try:
        history = await store.get_project_history(project_id, limit=limit, offset=offset)
        return ProjectHistoryResponse(**history)
    except Exception:
        logger.error("GET /api/projects/%s/history failed", project_id, exc_info=True)
...
async def get_project_history_summary(
    ...
    try:
        ...
        return _problem(500, "Failed to generate project history summary. Check server logs.")
