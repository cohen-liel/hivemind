    ...
    "todo_api": {
        ...
        "tasks": [
            ...
            TaskInput(
                ...
                acceptance_criteria=["test_app.py exists", "Tests pass"],
            ),
        ],
    },
    "calculator_cli": {
        ...
        "tasks": [
            ...
            TaskInput(
                ...
                depends_on=["task_001"],
                context_from=["task_001"],
                acceptance_criteria=["test_calculator.py exists", "Tests pass"],
            ),
        ],
    },
    "url_shortener": {
        ...
        "epics": [
            ...
            "Implement shortening and redirect",
            "Add analytics",
            "Write tests",
        ],
        "tasks": [
...
async def main():
    output_dir = os.path.join(
        str(HIVEMIND_ROOT), "benchmarks", "results", f"oss_langgraph_enhanced_{int(time.time())}"
    )
    ...
    for project_name, project_def in PROJECTS.items():
        ...
        subprocess.run(
            ["git", "config", "user.name", "Bench"], cwd=project_dir, capture_output=True
        )
        ...
        try:
            ...
            results.append(
                {"project": project_name, "variant": "langgraph_enhanced", "error": str(e)}
            )
            continue
        ...
        with open(
            os.path.join(output_dir, f"langgraph_enhanced_{project_name}_results.json"), "w"
        ) as f:
    ...
    for r in results:
        if "error" in r:
            ...
            continue
        print(
            f"{r['project']:<18} {r['tasks_succeeded']}/{r['tasks_total']:<6} "
