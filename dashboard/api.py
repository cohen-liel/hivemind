...
from dashboard.routers import event_bus  # noqa: F401
...
def create_app() -> FastAPI:
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        ...
        logger.warning(
            ...
            request_id, request.method, request.url.path, detail,
        )
        return JSONResponse(
    ...
    async def http_exception_handler(request: Request, exc: _HTTPException):
        ...
        logger.warning(
            "[%s] HTTP %d on %s %s: %s",
            request_id, status, request.method, request.url.path, detail,
        )
        return JSONResponse(
    ...
    async def body_size_limit(request: Request, call_next):
        ...
        if content_length:
            ...
            if cl > _MAX_BODY_SIZE:
                return _problem(
                    413, f"Request body too large. Maximum is {_MAX_BODY_SIZE // 1024}KB.",
                )
    ...
    _RATE_LIMIT_EXEMPT = {
        "/api/health", "/api/ready", "/api/stats",
        "/api/agent-registry", "/api/projects", "/health",
    }
    ...
    async def rate_limit_middleware(request: Request, call_next):
        ...
        if len(timestamps) >= _RATE_LIMIT_MAX_REQUESTS:
            logger.warning(
                ...
                client_ip, len(timestamps), _RATE_LIMIT_WINDOW, request.method, path,
            )
            return _problem(
        ...
        if recent_burst >= burst_limit:
            ...
            return _problem(
                429, "Too many requests in a short time. Please wait a moment.",
