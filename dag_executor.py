...
async def _emit_checkpoint(
    ...
    *,
    ...
    checkpoint = DAGCheckpoint(
        ...
        completed_tasks={
            tid: out.model_dump() for tid, out in ctx.completed.items()
        },
...
def _force_retry_stuck_tasks(ctx: _ExecutionContext, graph: TaskGraph) -> int:
    ...
    for task in graph.tasks:
        ...
        deps_ok = all(
            dep in ctx.completed and ctx.completed[dep].is_successful()
        ...
        retried += 1
        logger.info(
            f"[DAG] Force-retry: {task.id} (attempt {ctx.retries[task.id]}/{max_allowed})"
        )
...
async def restore_graph_from_checkpoint(
    ...
    try:
        ...
        completed = {
            tid: TaskOutput.model_validate(data)
            for tid, data in checkpoint.completed_tasks.items()
        }
        logger.info(
