...
from contracts import TaskInput, TaskOutput, TaskStatus, FailureCategory, AgentRole
from dynamic_spawner import DynamicSpawner, SpawnAttempt, DEFAULT_MODEL_CASCADE
...
class TestShouldRespawn:
    ...
    def test_all_models_exhausted(self):
        ...
        task = _make_task()
        output = _make_output(summary="Build error")
...
class TestGetRespawnModel:
    def test_returns_first_untried_model(self):
        spawner = DynamicSpawner()
        task = _make_task()
        output = _make_output(summary="Build error: compile failed")
    ...
    def test_skips_current_model(self):
        ...
        task = _make_task()
        output = _make_output(summary="Build error: compile failed")
        model = spawner.get_respawn_model(
            task, output, current_model=DEFAULT_MODEL_CASCADE[0]
        )
...
class TestRecordResult:
    ...
    def test_updates_correct_attempt(self):
        ...
        t1 = _make_task("t1")
        t2 = _make_task("t2")
        output = _make_output(summary="Build error", task_id="t1")
        output2 = _make_output(summary="Build error", task_id="t2")
        m1 = spawner.get_respawn_model(t1, output)
...
class TestClassifyFailure:
    ...
    def test_default_to_build_error(self):
        spawner = DynamicSpawner()
