...
from file_output_manager import ArtifactRegistry, ArtifactRef, infer_file_type


# ── Helpers ──────────────────────────────────────────────────────────────────

def _make_output(task_id: str, artifacts: list[str], status=TaskStatus.COMPLETED,
                 structured: list[Artifact] | None = None) -> TaskOutput:
    return TaskOutput(
        task_id=task_id,
        status=status,
        summary="Done",
...
def _make_task(task_id: str, context_from: list[str] | None = None,
               input_artifacts: list[str] | None = None) -> TaskInput:
    return TaskInput(
        id=task_id,
