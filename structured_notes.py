...
class StructuredNotes:
    ...
    def init_session(self, goal: str) -> None:
        ...
        if os.path.exists(self.notes_json_path):
            try:
                with open(self.notes_json_path, "r") as f:
                    data = json.load(f)
    ...
    def add_note(
        ...
        logger.info(
            ...
            note.id, category.value, title, author_role,
        )
        return note
    ...
    def get_relevant_notes(
        ...
        if tags:
            ...
            filtered = [
                n for n in filtered
                if any(t.lower() in tag_set for t in n.tags)
            ]
        ...
        if not categories and not tags and task_goal:
            ...
            # Remove common words
            stop_words = {"the", "a", "an", "is", "are", "for", "to", "and", "or", "of", "in", "on", "with"}
            keywords -= stop_words
